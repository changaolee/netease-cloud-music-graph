...
class NeteaseCloudMusic(object):
    ...
    __NETEASE_HOST = 'http://music.163.com'
    ...
    def __repr__(self):
        return '<NeteaseCloudMusic [%s]>' % (self.method)
    ...
    def _get_webapi_requests(self):
        ...
        NeteaseCloudMusic.req.headers.update(headers)
        return NeteaseCloudMusic.req
    ...
    def _build_response(self, resp):
        ...
        #     cookiesJar.save_cookies(resp, NeteaseCloudMusic.username)
    ...
    def send(self):
        ...
        try:
            if self.method == 'SEARCH':
                ...
                _url = self.__NETEASE_HOST + self._METHODS[self.method]
                resp = req.post(_url, data=self.data)
                self._build_response(resp)
                self.response.ok = True
            else:
                if isinstance(self.data, dict):
                    data = Encrypt.encrypted_request(self.data)
...
class Response(object):
    ...
    def json(self):
        ...
        if not self.headers and len(self.content) > 3:
            encoding = Encrypt.get_encoding_from_headers(self.headers)
            if encoding is not None:
                return json.loads(self.content.decode(encoding))
        return json.loads(self.content)


class NeteaseCloudMusicRequest(object):

    @classmethod
    def login(cls, password, phone=None, email=None, rememberLogin=True):
        """
    ...
    @classmethod
    def user_playlist(cls, uid, offset=0, limit=1000):
        """
        获取用户歌单，包含收藏的歌单
    ...
    @classmethod
    def user_dj(cls, uid, offset=0, limit=30):
        """
        获取用户电台数据
    ...
    @classmethod
    def search(cls, keyword, type=1, offset=0, limit=30):
        """
        搜索歌曲，支持搜索歌曲、歌手、专辑等
    ...
    @classmethod
    def user_follows(cls, uid, offset='0', limit=30):
        """
        获取用户关注列表
    ...
    @classmethod
    def user_followeds(cls, uid, offset='0', limit=30):
        """
        获取用户粉丝列表
    ...
    @classmethod
    def user_record(cls, uid, type=0):
        """
        获取用户的播放列表,必须登录
    ...
    @classmethod
    def top_playlist_highquality(cls, cat='全部', offset=0, limit=20):
        """
        获取网易云音乐的精品歌单
    ...
    @classmethod
    def play_list_detail(cls, playlist_id, limit=1000):
        """
        获取歌单中的所有音乐。由于获取精品中，只能看到歌单名字和 ID 并没有歌单的音乐，因此增加该接口传入歌单 ID
        获取歌单中的所有音乐.

        :param playlist_id: 歌单的ID
        :param limit: (optional) 数据上限多少行，默认 20
        """
        ...
        r.data = {'id': playlist_id, 'limit': limit, "csrf_token": ""}
        r.send()

        return r.response.json()

    @classmethod
    def music_url(cls, ids=[]):
        """
        通过歌曲 ID 获取歌曲下载地址
    ...
    @classmethod
    def music_comment(cls, song_id, offset=0, limit=20):
        """
        获取歌曲的评论列表
        ...
        r.params = {'id': song_id}
        r.data = {'offset': offset, 'limit': limit, 'rid': song_id, "csrf_token": ""}
        r.send()

        return r.response.json()

    @classmethod
    def album_comment(cls, album_id, offset=0, limit=20):
        if album_id is None:
            raise ParamsError()
        r = NeteaseCloudMusic()
        r.method = 'ALBUM_COMMENT'
        r.params = {'id': album_id}
        r.data = {'offset': offset, 'limit': limit, 'rid': album_id, "csrf_token": ""}
        r.send()

        return r.response.json()

    @classmethod
    def song_detail(cls, ids):
        """
        通过歌曲 ID 获取歌曲的详细信息
...
class NeteaseCloudMusicException(Exception):
    """
    这是 NeteaseCloudMusic 当处理请求时候的异常
    """


class ParamsError(NeteaseCloudMusicException):
    """
    参数错误
    """


class InvalidMethod(NeteaseCloudMusicException):
    """
    不支持的方法被调用
    """
